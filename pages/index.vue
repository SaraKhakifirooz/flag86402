<template>
    <div class="w-full p-5">
        ...
        <!-- <CustomInput />
        <br>
        let's go to {{ countryName }}
        <br>

        <button class="bg-red-200 px-3" @click="countryName = ''">reset</button>
        <br>
        <fput v-model="countryName" name="first_name" id="fname" label="First Name" />
        <div class="container">
            <div class="w-full bg-gray-100 p-3">
                <div class="w-full bg-gray-200 p-3 mb-3">
                    <h1>Flag</h1>
                </div>
                <div class="w-full flex bg-red-400">
                    <div class="relative  mr-3">
                        <select name="" id="" v-model="selectedCountry" class="border-l-2 p-2 w-[200px] rounded">
                            <option value="" v-for="item, i in uniqshow" :key="i">
                                {{ item }}
                            </option>
                        </select>
                    </div>
                    <div class="relative  mr-3">
                        ...
                        <input type="text" placeholder="continent" class="w-[200px] border rounded p-2">

                        <svg class="absolute right-2 bottom-2 border-l-2 pl-2" width="30px" height="30px" version="1.1"
                    ...
                    <div class="relative ">
                        <input type="text" placeholder="Search" class="w-[200px] border rounded p-2">
                        <svg class="absolute right-2 bottom-2 border-l-2 pl-2" width="30px" height="30px"
                ...
                <div class="w-full bg-gray">
                    <ul class="flex flex-wrap">
                        <flag v-for="(item, i) in result" :key="i" :info="item"  class="flag"/>
                    </ul>
                </div>
            </div>
        </div> -->
...
// const filterMovies = () => {
//     selectedValue.value = _.filter(uniqshow, function (o) {
//         return o.result.includes(selectedGenre.value);
//     })
// }
...
// const flatMap = _.flatMap(result, 'continents')
// const uniqshow = _.uniq(flatMap)
